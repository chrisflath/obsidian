...
Cross-platform replication using Freestyle Friday (960 blitz 3+1) vs Titled Tuesday (standard blitz 3+1). Updated with N=5,182 games (2,481 960, 2,701 std), 252 paired players. Engine analysis still in progress (~15K games total expected).
...
The format gap is ~1.87x larger on chess.com (blitz elite vs rapid general). The *share* explained by position features is remarkably similar (~36-41%), supporting structural replication.
...
Queen overtakes bishops as the largest single contributor in the elite sample. Knights flip from negative (Lichess) to slightly positive — possibly because titled players are better at exploiting non-standard knight positions in standard chess.
...
Clock state is zero because chess.com clock data is not yet extracted from PGN comments.
...
**Script**: Inline analysis run against `db/chesscom.db` with backfilled SP numbers from FEN.
