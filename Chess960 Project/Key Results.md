...
3. **Fig 3** (`fig3_blup_scatter.pdf`): Two-panel calibration figure — (a) per-player calibration shift violins by ELO tier (|eval_pv1−pv5| × time correlation, 960 minus standard), (b) error concentration by position quality × format (demanding/moderate/forgiving bars)
