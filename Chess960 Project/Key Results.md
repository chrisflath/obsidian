...
## Result 12: Cross-Platform Replication (Chess.com Elite, N=5,182)

From [[Freestyle Friday Data]] and [[Gelbach Decomposition]]:

Using Freestyle Friday (960 blitz 3+1) vs Titled Tuesday (standard blitz 3+1), N=5,182 games (2,481 960 / 2,701 std), 252 paired players:
...
- **Queen** is largest single contributor at 17.0% (vs bishops 15.4% in Lichess) — queen overtakes bishops in elite sample
- **Bishops** second at 9.8%, **unprotected pawns** amplified to 8.0%
- **Template distance gradient** replicates within 960 (chess.com slope 0.0075 vs Lichess 0.0057)

### Theta Decomposition (chess.com)

- **theta3 NULL with API ratings** (-0.004, p=0.60) due to range restriction (R_std SD=240 vs Lichess 435)
- **Partial recovery with in-game ratings**: theta3 = +0.019* (p=0.034)
- **Phase gradient inverted**: theta4 strengthens in middle/endgame (-0.030**, -0.060**) rather than opening
...
## Result 15: Expertise Gradient in Template Distance (Panel B Quintiles)

From the cross-platform graded transfer analysis:

The template distance gradient **steepens monotonically with expertise**:

| Tier | Rating range | Slope | Offset (jump at td=0) |
|------|-------------|-------|-----------------------|
| Q1 (weakest) | 618–1420 | 0.0031 | +0.099 |
| Q2 | 1427–1648 | 0.0038 | +0.083 |
| Q3 | 1650–1862 | 0.0047 | +0.090 |
| Q4 | 1864–2131 | 0.0061 | +0.107 |
| Q5 (strongest) | 2136–3085 | 0.0083 | +0.140 |
| Titled (Blitz) | Chess.com GMs/IMs | 0.0075 | +0.210 |

- Slope steepens **2.6x** from Q1 to Q5 — stronger players are more sensitive to template distance
- Chess.com titled slope (0.0075) is comparable to Q5 despite different time control (blitz vs rapid)
- The format **offset** (jump at td=0, reflecting format disruption beyond displacement) also grows with expertise: +0.099 (Q1) → +0.210 (titled)
- This is consistent with Result 13: template dependency deepens with expertise

> [!important] Template sensitivity scales with investment
> Weaker players lose roughly the same amount regardless of how displaced the position is. Strong players' performance degrades sharply with displacement — they've invested more in position-specific templates, so they lose more when those templates are disrupted.

**Figure**: `plots/publication/panel_b_quintiles.pdf` — 6-tier graded transfer plot with marginal histogram and back-rank vignettes.
...
4. **Panel B quintiles** (`panel_b_quintiles.pdf`): 6-tier expertise gradient (5 Lichess quintiles + chess.com titled) — candidate for extended figure or appendix
